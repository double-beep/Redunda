class BotInstancesController < ApplicationController
  ...
  before_action :authenticate_user!, only: %i[new create edit update destroy revoke_key]
  before_action :set_bot_instance, only: %i[edit update destroy reorder revoke_key]
  before_action :set_bot_instance_from_key, only: %i[status_ping show_events]
  before_action :set_bot, except: %i[status_ping show_events]
  before_action :check_instance_permissions, only: %i[edit update destroy revoke_key]
  before_action :check_bot_permissions, only: %i[new create]
  ...
  def create
    ...
    respond_to do |format|
      if @bot_instance.save
        format.html do
          redirect_to url_for(controller: :bot_instances, action: :index, bot_id: params[:bot_id]),
  ...
  def reorder
    ...
    unless user_signed_in? && (
      ...
      respond_to do |format|
        format.html do
          redirect_to bot_bot_instances_url(bot_id: params[:bot_id]),
                      flash: { error: 'You do not have permission to move that instance.' }
        end
      end
      return
    end
    ...
    if other_instance.nil?
      respond_to do |format|
        format.html do
          redirect_to bot_bot_instances_url(bot_id: params[:bot_id]),
                      flash: { error: 'That instance may not be moved in that direction.' }
        end
      end
      return
    end
  ...
  def check_bot_permissions
    return if current_user.is_owner?(@bot) || current_user.is_collaborator?(@bot) || current_user.is_admin?
